...
async function main() {
  ...
  try {
    ...
    // ================= LOGIN (opcional no build offline/demo) =================
    ...
    // build offline/demo (pos-LCP-fix) entra direto no app: clica guest se existir
    const guestClicked = await waitFor(`(() => { const b=[...document.querySelectorAll('button')].find(b=>/guest|convidado/i.test(b.textContent||b.getAttribute('aria-label')||'')); if(b){b.click();return true}return false})()`, 6, 'guest-opcional');
    console.log('[login] guest clicado (false = demo direto):', !!guestClicked);
    await sleep(2500);
